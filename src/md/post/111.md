...
もし先のなるだない段落が規範的な解説ではなく、単なる例の一つとして挙げているにすぎないのであれば、この結論は表題の限りではなくなる。

わざわざ <q lang="en">As a general rule, ~</q> と前置きしたり should を使っていたり、一応非規範的ですよ的な含みを感じてしまうのだ。

例えば、まだ HTML に定義されていないセマンティクスを備え得る UI（タブやドロワーなど）を作る時、俺たちはどうしたって謎の div 構造を書いている。

それは WAI-ARIA で補強するためにグルーピングした div だったり、レイアウトや装飾目的での空 div だったりするだろう。HTML 仕様はそういった現状の HTML 要素群では表現できないウェブコンテンツに対しても、変わらずに触知可能であることを仕様として div に求めているのだろうか？　無理があるのではないか？

と同時に、なるほだない段落がもし非規範的であるならば、仕様書が他の説明でしているように <q lang="en">non-normative</q> ーー非規範的、と書いているはずとも思う。そう書いていないのだから、これはやはり規範的な説明なのだという考えに戻ってくる。

いや、だがしかし。本当にそうか？　やはり規範的だったとして、俺たちは一体なんの要素で空 ◯◯ を作ればいいんだ？
...
| 要素間の空白でない<br>テキスト                               | -                | パルパブルコンテンツには空白は入れられない                                                                                                                                                                                         |
| 自律的な<br>カスタムエレメント                               | ?                |                                                                                                                                                                                                                                    |
...
ここでは、レイアウトや装飾目的の空 div を代替する虚無な要素を `<void-fill>` というタグ名で作っていく。本当は `void` とか `empty`とか `spacer`のような一語のタグにしたいが、カスタムエレメントは未来の新要素との衝突を防ぐために名前にハイフンを含めなくてはならないのでそれはできない。
...
終わり。小さく使う分には何も難しいことはないように感じる。CSS クラス名を作りたくないというならば、CSS のカスタムプロパティを使って、
...
実際のところ、空 div を使いたいシーンにおいて現場ではカスタムエレメントなんていちいち定義してなどいられない。せめて仕様に一言、div と span のコンテンツモデルに "nothing" が付け加えられればいいのに。それだけで俺の世界は平和になる。さすれば HTML は CSS から解き放たれ、大地に深く根を張り、その身は雄々しく聳え立ち、俺に永遠の安らぎを与えるだろう。
...
Special Thanks! [@GeckTang](https://twitter.com/geckotang), [@glatyou](https://twitter.com/glatyou), [@\_tsmd](https://twitter.com/_tsmd)!
