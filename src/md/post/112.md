...
>
> <cite>[ WHATWG HTML Standard - 3.2.1 Semantics](https://html.spec.whatwg.org/multipage/dom.html#semantics-2)</cite>
...
<table>
  ...
  <tbody>
    <tr>
      <td>カテゴリー</td>
      <td>フローコンテンツ、パルパブルコンテンツ</td>
      <td>フローコンテンツ、フレージングコンテンツ、パルパブルコンテンツ</td>
    </tr>
    <tr>
      <td>コンテンツモデル</td>
      <td>div が dl の子要素の場合、1 つ以上の dd にフォローされる 1 つ以上の dt 要素（さらにスクリプトサポーティング要素があっても良い）を内包できる。そうない場合、フローコンテンツを内包できる。</td>
      <td>フレージングコンテンツを内包できる</td>
    </tr>
    <tr>
      <td>フローコンテンツとしての特性</td>
      <td colspan="2"><q lang="en">text</q> （テキストノードがなくてもいい。ASCII 空白文字の場合は要素間の空白になれる）を内包できる</td>
    </tr>
    <tr>
      <td>フレージングコンテンツとしても特性</td>
      <td>-</td>
      <td><q lang="en">text</q> （テキストノードがなくてもいい。ASCII 空白文字の場合は要素間の空白になれる）を内包できる</td>
    </tr>
    <tr>
      <td>パルパブルコンテンツとしての特性</td>
      <td colspan="2">中身は触知可能なコンテンツでかつ要素間の空白ではないテキストを内包しなくてはいけないが、空であることに正当性がある場合は厳しく要求はしない</td>
    </tr>
  </tbody>
</table>

ふむ。ここで div 要素仕様から Note を引用したい。
...
長々と語ってきたフローコンテンツやフレージングコンテンツ、<q lang="en">text</q>の定義は今回の話ではどうでもよかった。最終的な争点としては、
...
- パルパブルコンテンツに準じれば、空であることに正当性がある場合は、内包物が触知可能でなくてはならない縛りは適用されず、div や span は空にできる
  - では、正当性とは何か？
- div 要素に準じれば、もう他に手段がない場合には div を使えと div が言っている
