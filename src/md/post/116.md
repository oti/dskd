...
gulp-imagemin に問題がないとわかれば、gulp を ESM で動作させる他の方法を検証すればよい。そうして記事前半の解決方法の提示に至よた。
...
そして `fs.readFileSync()` はファイルシステムとしての read しかしてくれず、それを JSON として扱うには `JSON.parse()` を経由する必要がある。さらに `JSON.parse()` はパースできないと例外を返すので `try catch` もしなくてはならない。こういった変更は ESM 化するなら避けられない作業なので頑張ろう。
