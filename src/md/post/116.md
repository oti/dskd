...
<table class="-align-start">
  ...
  <tbody>
    ...
    <tr>
      <th scope="row">package.json に "type": "module" を追加する</th>
...
いずれ `"type": "module"` のオプションは外され、Node.js も ESM がデフォルトになるだろう。いつのバージョンでそうなるかはわからないが、JavaScript をとりまく潮流として ESM 化は避けられない。Node.js v20 あたりがキリがいいのでそこでそうなるかもしれない。ライフサイクル的には来年の 10 月で v20 がアクティブ LTS になるはずだ。

もしそうならあまり時間はない。gulp-imagemin に限らず、他の gulp プラグインや node プラグインもそう時待たずに ESM 化していくだろう。
