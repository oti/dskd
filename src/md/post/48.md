...
[さっそく一通りのブラウザで表示確認](/image/overflow-overlay/mac_preferences.webp)する。前提として、標準設定の Mac OS とモバイルブラウザはスクロールバーがデフォルトで非表示であり、コンテンツがスクロール可能な場合はスクロールするとオーバーレイでスクロールバーが表示される。なのでそちらのキャプチャは撮っていない。Mac で確認したい場合は、環境設定 -> 一般 -> スクロールバーを「常に表示」にすると良い。
