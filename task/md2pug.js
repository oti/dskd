...
(function () {
  ...
  p.render = function (src) {
    this.md.use(
      typeof module !== "undefined" && typeof exports === "object"
        ? require("./pug-mixin")
        : window.MarkdownToPug.mixin
    );
    this.md.use(
      typeof module !== "undefined" && typeof exports === "object"
        ? require("./pug-template")
        : window.MarkdownToPug.template
    );
      ...
      switch (e.type) {
        ...
        case "td_open":
          if (e.hidden) {
            ...
            ret += this.indent(
              this.tag(e.tag, e.content, e.attrs),
              e.level + level
            );
          }
          break;
        ...
        case "fence":
          ret += this.indent(
            this.code(e.tag, e.content, e.attrs, e.info),
            e.level + level
          );
          break;
        case "pugjs-mixin":
          ...
          if (e.children) {
            ...
            e.children.forEach((c) => {
              ...
              switch (c.type) {
                ...
                case "code_inline":
                  ret += this.indent(
                    this.htmlTag(c.tag, c.content, c.attrs),
                    e.level + level
                  );
                  break;
                case "strong_open":
                ...
                case "image":
                  ret += this.indent(
                    this.tag(c.tag, c.content, c.attrs),
                    e.level + level
                  );
                  break;
  ...
  p.htmlTag = function (tag, src, attrs) {
    ...
    let attributes =
      (attrs != null &&
        attrs != undefined &&
        " " + attrs.map((e) => e[0] + '="' + e[1] + '"').join(" ")) ||
      "";
    return `<${tag}${attributes}>${he.encode(src, {
  ...
  p.code = function (tag, src, attrs, info) {
    ...
    return (
      `pre.hljs${info}${this.options.newline}${this.options.space}` +
